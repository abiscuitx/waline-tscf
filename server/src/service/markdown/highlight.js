...
const loadLanguages = (languages = []) => {
  ...
  // 过滤出尚未加载的语言
  const langsToLoad = languages.filter((item) => !prismInstance.languages[item]);
...
const resolveHighlighter = (language) => {
  ...
  // 返回高亮处理函数
  return (code) => prismInstance.highlight(code, prismInstance.languages[language], language);
};
